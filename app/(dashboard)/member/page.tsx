...
export default async function MemberHomePage() {
  ...
  // Expiry check
  const expiryDate = m.membership_expiry ? new Date(m.membership_expiry) : null;
  const now = new Date();
  const daysRemaining = expiryDate ? Math.ceil((expiryDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)) : 0;
  const isExpiringSoon = daysRemaining <= 7 && daysRemaining >= 0;
  const isExpired = daysRemaining < 0 || m.status === "EXPIRED";

  const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const currentDayName = dayNames[new Date().getDay()];
  const fallbackRoutine = workoutPlan?.days?.find((d: any) => d.day.toLowerCase() === currentDayName.toLowerCase()) || workoutPlan?.days?.[0];

  const workoutTitle = todaySchedule?.title || fallbackRoutine?.focus;
  const workoutExercises = todaySchedule?.exercises || fallbackRoutine?.exercises;
  const isWorkoutCompleted = todaySchedule?.status === "COMPLETED";
  ...
  return (
    <div className="space-y-6">
      {/* 0. Prominent Membership Payment Due Alert */}
      {(isExpiringSoon || isExpired) && (
        <section className={`p-4 rounded-lg border flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm ${
          isExpired
          ...
          <div>
            <span className="text-xs font-bold uppercase tracking-wider block">
              {isExpired ? "Membership Expired" : `Membership Expiring in ${daysRemaining} Day${daysRemaining === 1 ? '' : 's'}`}
            </span>
            <p className="text-xs mt-0.5 opacity-90">
              {isExpired
                ? `Your gym access expired on ${formatDate(m.membership_expiry)}. Renew now to maintain uninterrupted floor access.`
                : `Your subscription will expire on ${formatDate(m.membership_expiry)}. Renew early without losing remaining days.`}
            </p>
          </div>
          <Link
            href="/member/payments"
            className="h-8 px-4 bg-[#1E40AF] text-white hover:bg-blue-800 text-xs font-semibold rounded flex items-center justify-center gap-1.5 transition-colors shrink-0 shadow-sm"
          >
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-5 flex flex-col justify-between shadow-sm space-y-4">
          <div>
            ...
            {workoutTitle ? (
              <div className="mt-3">
                <h2 className="text-lg font-bold text-slate-900 mb-1">{workoutTitle}</h2>
                <p className="text-xs text-slate-500 mb-3">
                  {workoutExercises?.length || 4} targeted exercises with progressive overload targets.
                </p>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {workoutExercises?.slice(0, 4).map((ex: any, idx: number) => (
                    <span
                      key={idx}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            <span className="text-xs text-slate-500 font-mono">
              {isWorkoutCompleted ? "Session recorded" : "Ready on gym floor"}
            </span>
            <Link
              href="/member/workout"
              className={`h-8 px-4 text-xs font-semibold rounded flex items-center gap-1.5 transition-colors shadow-sm ${
                isWorkoutCompleted
