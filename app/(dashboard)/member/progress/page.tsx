            ...
            {records && records.length > 0 ? (
              records.map((r: any) => (
                <tr key={r.id} className="text-slate-800 hover:bg-slate-50/60 transition-colors">
                  <td className="px-5 py-3.5 font-medium text-slate-500">{formatDate(r.recorded_at)}</td>
                  <td className="px-5 py-3.5 font-bold text-slate-900 tabular-nums">{r.weight_kg ? `${r.weight_kg} kg` : "—"}</td>
                  <td className="px-5 py-3.5 tabular-nums">{r.chest_inches ? `${r.chest_inches}"` : "—"}</td>
                  <td className="px-5 py-3.5 tabular-nums">{r.waist_inches ? `${r.waist_inches}"` : "—"}</td>
                  <td className="px-5 py-3.5 font-semibold text-emerald-700 tabular-nums">{r.bench_press_kg ? `${r.bench_press_kg} kg` : "—"}</td>
                  <td className="px-5 py-3.5 font-semibold text-blue-800 tabular-nums">{r.squat_kg ? `${r.squat_kg} kg` : "—"}</td>
                </tr>
              ))
            ) : (
              <tr>
