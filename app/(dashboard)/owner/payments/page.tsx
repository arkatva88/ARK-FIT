...
export default async function OwnerPaymentsPage({ searchParams }: PageProps) {
  ...
  // Parallelize financial stats, payments ledger, and active members in a single network roundtrip
  const [
    { data: allPayments },
    { data: payments },
    { data: activeMembers }
    ...
    query,
    supabase.from("members").select("id, profiles(full_name)").eq("status", "ACTIVE")
  ]);
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
          ...
          <p className="mt-1 text-[11px] text-slate-500">Realized cash & verified online payments</p>
        </div>

        {/* Card 2: Pending Receivables */}
        <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-amber-700 uppercase tracking-wider">Pending Dues</span>
            <span className="inline-block w-2 h-2 rounded-full bg-amber-500"></span>
          </div>
          <div className="mt-2 text-2xl font-bold text-amber-800 tabular-nums">{formatCurrency(pending)}</div>
          <p className="mt-1 text-[11px] text-slate-500">Scheduled invoices awaiting payment</p>
        </div>

        {/* Card 3: Overdue Amount */}
        <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-rose-700 uppercase tracking-wider">Overdue Balance</span>
          ...
          <p className="mt-1 text-[11px] text-slate-500">Past due grace period threshold</p>
        </div>
      </section>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        {filterTabs.map((tab) => (
          <Link
            ...
            className={`h-8 px-3.5 rounded text-xs font-semibold transition-colors flex items-center shrink-0 ${
              statusFilter === tab.id
